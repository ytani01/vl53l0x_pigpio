...
from vl53l0x_pigpio.constants import (
    SYSRANGE_START, SYSTEM_SEQUENCE_CONFIG, SYSTEM_INTERRUPT_CONFIG_GPIO, GPIO_HV_MUX_ACTIVE_HIGH,
    SYSTEM_INTERRUPT_CLEAR, RESULT_INTERRUPT_STATUS, RESULT_RANGE_STATUS,
    MSRC_CONFIG_CONTROL, FINAL_RANGE_CFG_MIN_COUNT_RATE_RTN_LIMIT, GLOBAL_CFG_SPAD_ENABLES_REF_0, GLOBAL_CFG_REF_EN_START_SELECT, DYN_SPAD_NUM_REQUESTED_REF_SPAD,
    DYN_SPAD_REF_EN_START_OFFSET, VALUE_00, VALUE_01, VALUE_02, VALUE_03, VALUE_04, VALUE_05, VALUE_06,
    VALUE_07, VALUE_08, VALUE_09, VALUE_0A, VALUE_10, VALUE_12, VALUE_14, VALUE_1A,
    VALUE_20, VALUE_21, VALUE_25, VALUE_26, VALUE_28, VALUE_30, VALUE_32,
    VALUE_34, VALUE_40, VALUE_44, VALUE_6B, VALUE_83, VALUE_96, VALUE_A0,
    VALUE_B4, VALUE_E8, VALUE_FE, VALUE_FF, VALUE_F8,

    REG_00, REG_01, REG_0D, REG_0E, REG_09, REG_10, REG_11, REG_20, REG_22,
    REG_23, REG_24, REG_25, REG_27, REG_30, REG_31, REG_32, REG_34, REG_35,
    REG_40, REG_42, REG_43, REG_44, REG_45, REG_46, REG_47, REG_48, REG_49,
    REG_4A, REG_4B, REG_4C, REG_4D, REG_4E, REG_50, REG_51, REG_52, REG_54,
    REG_56, REG_57, REG_60, REG_61, REG_62, REG_64, REG_65, REG_66, REG_67,
    REG_70, REG_71, REG_72, REG_75, REG_76, REG_77, REG_78, REG_7A, REG_7B,
    REG_80, REG_81, REG_8E, REG_91, REG_92, REG_94, REG_FF,

    I2C_STANDARD_MODE, SPAD_NUM_REQUESTED_REF, GPIO_INTERRUPT_CONFIG,
    CALIBRATION_VALUE_40, TIMEOUT_LIMIT
)
...
class VL53L0X:
    ...
    def _configure_signal_rate_limit(self):
        ...
        self.write_byte(MSRC_CONFIG_CONTROL, (
            self.read_byte(MSRC_CONFIG_CONTROL) | VALUE_12))

        # set final range signal rate limit to 0.25 MCPS (million counts per second)
        self.write_word(
            FINAL_RANGE_CFG_MIN_COUNT_RATE_RTN_LIMIT, VALUE_32) # 0.25 * 128 = 32
    ...
    def _setup_spad_info(self):
        ...
        ref_spad_map = self.read_block(
            GLOBAL_CFG_SPAD_ENABLES_REF_0, 6)

        self.write_byte(REG_FF, VALUE_01)
        self.write_byte(DYN_SPAD_REF_EN_START_OFFSET, VALUE_00)
        self.write_byte(DYN_SPAD_NUM_REQUESTED_REF_SPAD, SPAD_NUM_REQUESTED_REF)
        self.write_byte(REG_FF, VALUE_00)
        ...
        for i in range(48):
            if (i < first_spad_to_enable or
                ...
                ref_spad_map[i // 8] &= ~(
                    1 << (i % 8))
            elif ((ref_spad_map[i // 8] >> (i % 8))
                    & 0x1):
                spads_enabled += 1

        self.write_block(
            GLOBAL_CFG_SPAD_ENABLES_REF_0, ref_spad_map)
    ...
    def _configure_interrupt_gpio(self):
        ...
        self.write_byte(GPIO_HV_MUX_ACTIVE_HIGH, (
            self.read_byte(GPIO_HV_MUX_ACTIVE_HIGH) & ~VALUE_10)) # active low
    ...
    def _get_spad_info(self):
        ...
        self.write_byte(VALUE_83, (
            self.read_byte(VALUE_83) | VALUE_04))
    ...
    def perform_single_ref_calibration(self, vhv_init_byte):
        ...
        start = time.time()
        while ((self.read_byte(RESULT_INTERRUPT_STATUS) & 0x07)
               == VALUE_00):
            if time.time() - start > TIMEOUT_LIMIT:
                raise Exception("Timeout")
        self.write_byte(SYSTEM_INTERRUPT_CLEAR, VALUE_01)
    ...
    def get_range(self):
        ...
        range_mm = self.read_word(
            RESULT_RANGE_STATUS + VALUE_0A) # 10 is 0x0A

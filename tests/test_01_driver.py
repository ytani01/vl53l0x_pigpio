...
class TestVL53L0XDriver(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.mock_read_byte_data_returns = {
            ...
            VALUE_83: [
                ...
                VALUE_00,
                VALUE_01,
            ],  # Simulate VALUE_83 changing from 0 to 1 to break the loop
            ...
            RESULT_INTERRUPT_STATUS: lambda: [
                VALUE_01
            ],  # Return VALUE_01 immediately
            SYSRANGE_START: lambda: [
                VALUE_00
            ],  # Return VALUE_00 immediately after first read
        ...
        self.mock_pi.i2c_read_i2c_block_data.side_effect = (
            lambda handle, register, count: (
                (6, bytearray([0x00] * 6))
                if register == GLOBAL_CFG_SPAD_ENABLES_REF_0 and count == 6
                else (3, bytearray([0x11, 0x22, 0x33]))
                if register == 0x05 and count == 3
                # For test_read_block
                else (count, bytearray([0x00] * count))
            )
        )
        ...
        def i2c_read_byte_data_side_effect(handle: int, register: int) -> int:
            if register in self.mock_read_byte_data_returns:
                ...
                if callable(return_value):
                    return_value = (
                        return_value()
                    )  # Call the lambda to get a fresh list
        ...
        self.mock_pi.i2c_read_byte_data.side_effect = (
            i2c_read_byte_data_side_effect
        )

        self.mock_pi.i2c_write_byte_data.return_value = 0
        self.mock_pi.i2c_read_word_data.return_value = (
            0  # Default mock return
        )
    ...
    def test_get_ranges(self) -> None:
        ...
        with VL53L0X(self.mock_pi) as tof:
            ...
            self.assertTrue(
                np.array_equal(
                    ranges, np.array([1234, 1235, 1236, 1237, 1238])
                )
            )
            self.assertEqual(
                self.mock_pi.i2c_read_word_data.call_count, num_samples + 4
    ...
    def test_write_word(self) -> None:
        ...
        with VL53L0X(self.mock_pi) as tof:
            ...
            self.mock_pi.i2c_write_word_data.assert_called_with(
                1, 0x04, 0xCDAB
            )

    def test_read_block(self) -> None:
        mock_block_data = [0x11, 0x22, 0x33]
        self.mock_pi.i2c_read_i2c_block_data.return_value = (
            0,
            mock_block_data,
        )
        with VL53L0X(self.mock_pi) as tof:
            ...
            self.mock_pi.i2c_read_i2c_block_data.assert_called_with(
                1, 0x05, 3
            )
    ...
    def test_write_block(self) -> None:
        ...
        with VL53L0X(self.mock_pi) as tof:
            ...
            self.mock_pi.i2c_write_i2c_block_data.assert_called_with(
                1, 0x06, test_data
            )
